...
const validationRules = {
	...
	buisnessCategory: {
		...
		format: { pattern: "^[a-zA-Z ]{3,50}$", message: "^Please enter valid Buisness Category" }
	},
	buisnessSize: {
		presence: { message: '^Please enter a Buisness Size' },
		format: { pattern: "^[a-zA-Z ]{3,50}$", message: "^Please enter valid Buisness Size" }
	},
						...
						<View style={[cstyles.center, styles.bottomFooterRelative]}>
							<View style={cstyles.padder}>
								...
								<InputUnderLineIcon placeholder="Category & Market" icon="certificate"
									...
									onChangeText={(value) => this.handleValidationChange("buisnessCategory", value)}
								/>
								{this.state.buisnessCategoryError && <Text style={cstyles.errorText}>{this.state.buisnessCategoryError}</Text>}
								...
								<PickerUnderLineIcon
									...
									onValueChange={(value) => this.handleValidationChange("buisnessSize", value)}
								/>
								{this.state.buisnessSizeError && <Text style={cstyles.errorText}>{this.state.buisnessSizeError}</Text>}
