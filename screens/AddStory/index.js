...
import { insertmystory } from "../../_actions/auth";
import { bindActionCreators } from "redux";
import config from "../../_config";
import Loader from "../../components/Loader";

function MyStory({ navigation, user, insertmystory }) {
  ...
  handleStorySubmimt = () => {
    if (heading.length < 3) {
      Alert.alert("Roraa", "Please add a proper heading");
    } else if (desc.length < 10) {
      Alert.alert("Roraa", "Please be descriptive");
    } else {
      setLoader(true);
      const data = new FormData();
      data.append("u_id", user.u_id);
      data.append("mytitle", heading);
      data.append("description", desc);
      console.log(data);
      new Promise((rsl, rej) => {
        insertmystory(data, rsl, rej);
      })
        .then((res) => {
          setLoader(false);
          navigation.goBack();
        ...
        >
          ...
          <View>
            <TextInput
              ...
              placeholder="everyone want to read your story"
            />
          </View>
        </View>
