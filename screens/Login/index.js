...
class Login extends React.Component {
	...
	handleSubmit = () => {
		...
		this.setState({
			...
			this.props.dispatch(userActions.login(this.state.email, this.state.password)).then(res => {
				this.setState({
					formError: null,
					isFormBusy: false,
					// count:0
				...
				this.setState({
					...
					password: "",
					passwordError: null,
					modalShow: (errorData.code && errorData.code == 'unverified-email') ? true : false,
					unVerifiedEmailResponse: (errorData.code && errorData.code == 'unverified-email') ? errorData.data : null
					// count:0
				});
			})
				...
				<View style={[styles.topContainer]}>
					...
					<View style={[cstyles.center, styles.bottomFooterRelative]}>
						<View style={cstyles.padder}>
							...
							{
								this.state.formError && <Text style={[cstyles.errorText, { marginTop: 10, marginBottom: -10 }]}>{this.state.formError}</Text>
							}
							<ButtonGradient
