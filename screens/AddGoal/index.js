...
import { insertMyGoal } from "../../_actions/auth";
import { bindActionCreators } from "redux";
import config from "../../_config";
import Loader from "../../components/Loader";

function AddGoal({ navigation, user, insertMyGoal }) {
  ...
  const handleSubmitGoal = () => {
    if (desc.length < 3) {
      Alert.alert("Roraa", "Kindly enter your goal");
    } else {
      setLoader(true);
    ...
    <SafeAreaView style={cstyles.container}>
      <ScrollView style={[cstyles.container, cstyles.padder]}>
        <View style={[cstyles.row, cstyles.itemsEnd, { paddingVertical: 25 }]}>
          ...
          <Text style={[cstyles.ml_10, styles.mainText]}>My Goal</Text>
        </View>
        <View
          style={[
            cstyles.boxShadow,
