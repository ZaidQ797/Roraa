...
import { insertMyInterest } from "../../_actions/auth";
import { bindActionCreators } from "redux";
import config from "../../_config";
import Loader from "../../components/Loader";

function AddInterest({ navigation, user, insertMyInterest }) {
  const [desc, setDesc] = useState("");
  const [loader, setLoader] = useState(false);

  const handleSubmitInterest = () => {
    if (desc.length < 3) {
      Alert.alert("Roraa", "Kindly enter your Interest");
    } else {
      setLoader(true);
    ...
    <SafeAreaView style={cstyles.container}>
      <ScrollView style={[cstyles.container, cstyles.padder]}>
        <View style={[cstyles.row, cstyles.itemsEnd, { paddingVertical: 25 }]}>
          ...
          <Text style={[cstyles.ml_10, styles.mainText]}>My Interest</Text>
        </View>
        <View
          style={[
            cstyles.boxShadow,
